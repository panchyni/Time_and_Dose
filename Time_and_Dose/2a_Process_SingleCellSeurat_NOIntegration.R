...
# Merge All Data
seurat_all <- merge(seurat_A, y = c(seurat_B ,seurat_C), project = "Dose_and_Time_Unintegrated")
...
seurat_all <- ScaleData(seurat_all, vars.to.regress=c("S.Score","G2M.Score","Batch") ,verbose = FALSE)
seurat_all <- RunPCA(seurat_all, npcs = 15, verbose = FALSE, features = row.names(seurat_A))
seurat_all <- RunUMAP(seurat_all, reduction = "pca", dims = 1:15)
seurat_all <- FindNeighbors(seurat_all, reduction = "pca", dims = 1:15)
...
seurat_all@meta.data <- metadata

##########################

saveRDS(seurat_all,'SeuratData/Seurat_UnIntegrated_TimeAndCon_CellCycle_hg38.rds')
#seurat_all <- readRDS('SeuratData/Seurat_UnIntegrated_TimeAndCon_CellCycle_hg38.rds')

p1 <- DimPlot(seurat_all, reduction = "umap",group.by = "Batch")
p2 <- DimPlot(seurat_all, reduction = "umap",group.by = "Time")
p3 <- DimPlot(seurat_all, reduction = "umap", label = TRUE, repel = TRUE)
p4 <- DimPlot(seurat_all, reduction = "umap",group.by = "Phase")
