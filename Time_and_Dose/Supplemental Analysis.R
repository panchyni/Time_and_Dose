### Folder
setwd("E:/2021/Work_4_9_2020_StringentFiltering/")
...
######################################################################################################################
### Older Stuff, maybe drop this ###
######################################################################################################################

### Intersection with Integration Cluster 8 ###
length(row.names(cluster_0_data))#
# 1454

print(table(cluster_0_data$Batch))
print(table(cluster_0_data$Phase))
print(table(cluster_0_data$GBC_pM))

for (n in 0:12){
  seurat_cluster_integrated <- integration.combined.All@meta.data[integration.combined.All@meta.data$seurat_clusters==n,]
  overlap <- intersect(row.names(cluster_2_data),row.names(seurat_cluster_integrated))
  #print(n)
  #print(length(overlap))
  print(length(overlap)/length(row.names(cluster_2_data)))
  #print(table(seurat_cluster_integrated[overlap,]$Batch))
  print(table(seurat_cluster_integrated[overlap,]$Phase))
  #print(table(seurat_cluster_integrated[overlap,]$GBC_pM))
}

seurat_cluster_integrated_0 <- integration.combined.All@meta.data[integration.combined.All@meta.data$seurat_clusters==0,]
seurat_cluster_integrated_5 <- integration.combined.All@meta.data[integration.combined.All@meta.data$seurat_clusters==5,]
seurat_cluster_integrated_8 <- integration.combined.All@meta.data[integration.combined.All@meta.data$seurat_clusters==8,]

for (n in 0:6){
  dose_only_cluster <- ConCC_integration.combined@meta.data[ConCC_integration.combined@meta.data$seurat_clusters == n,]
  overlap <- intersect(row.names(seurat_cluster_integrated_0),row.names(dose_only_cluster))
  print(length(overlap)/length(row.names(seurat_cluster_integrated_0)))
}

for (n in 0:6){
  dose_only_cluster <- ConCC_integration.combined@meta.data[ConCC_integration.combined@meta.data$seurat_clusters == n,]
  overlap <- intersect(row.names(seurat_cluster_integrated_5),row.names(dose_only_cluster))
  print(length(overlap)/length(row.names(seurat_cluster_integrated_5)))
}

for (n in 0:6){
  dose_only_cluster <- ConCC_integration.combined@meta.data[ConCC_integration.combined@meta.data$seurat_clusters == n,]
  overlap <- intersect(row.names(seurat_cluster_integrated_8),row.names(dose_only_cluster))
  print(length(overlap)/length(row.names(seurat_cluster_integrated_8)))
}



#################################################################
### Time Data Only ---  Cluster 1,6 Overlap #####################
#################################################################

Time_integration.combined <- readRDS("SeuratData/Seurat_integrated_TimeOnly_CellCycle.rds")
seurat_combo_Time_mat <- GetAssayData(object = Time_integration.combined, slot = "scale.data")

p_time_batch <- DimPlot(Time_integration.combined, reduction = "umap",group.by = "Batch")
p_time_time <- DimPlot(Time_integration.combined, reduction = "umap",group.by = "Time")
p_time_cluster <- DimPlot(Time_integration.combined, reduction = "umap", label = TRUE, repel = TRUE)
p_time_phase <- DimPlot(Time_integration.combined, reduction = "umap",group.by = "Phase")

# Integrated Cluster 1 #
seurat_cluster_integrated_1 <- integration.combined.All@meta.data[integration.combined.All@meta.data$seurat_clusters==1,]

for (n in 0:10){
  time_only_cluster <- Time_integration.combined@meta.data[Time_integration.combined@meta.data$seurat_clusters == n,]
  overlap <- intersect(row.names(seurat_cluster_integrated_1),row.names(time_only_cluster))
  print(length(overlap)/length(row.names(seurat_cluster_integrated_1)))
}

seurat_cluster_integrated_3 <- integration.combined.All@meta.data[integration.combined.All@meta.data$seurat_clusters==3,]

for (n in 0:10){
  time_only_cluster <- Time_integration.combined@meta.data[Time_integration.combined@meta.data$seurat_clusters == n,]
  overlap <- intersect(row.names(seurat_cluster_integrated_3),row.names(time_only_cluster))
  print(length(overlap)/length(row.names(seurat_cluster_integrated_3)))
}

seurat_cluster_integrated_6 <- integration.combined.All@meta.data[integration.combined.All@meta.data$seurat_clusters==6,]

for (n in 0:10){
  time_only_cluster <- Time_integration.combined@meta.data[Time_integration.combined@meta.data$seurat_clusters == n,]
  overlap <- intersect(row.names(seurat_cluster_integrated_6),row.names(time_only_cluster))
  print(length(overlap)/length(row.names(seurat_cluster_integrated_6)))
}
...
# For annotation data
seuratTime_combo_E <- ScaleData(Time_integration.combined, verbose = FALSE, vars.to.regress=c("S.Score","G2M.Score"), features=E_genes$Gene)
seuratTime_combo_M <- ScaleData(Time_integration.combined, verbose = FALSE, vars.to.regress=c("S.Score","G2M.Score"), features=M_genes$Gene)

# For consistent gene coverage
seuratTime_combo_E_mat <- seurat_combo_Time_mat[E_genes$Gene[E_genes$Gene %in% row.names(seurat_combo_Time_mat)],]
seuratTime_combo_M_mat <- seurat_combo_Time_mat[M_genes$Gene[M_genes$Gene %in% row.names(seurat_combo_Time_mat)],]

dim(seuratTime_combo_E_mat)
dim(seuratTime_combo_M_mat)

# E
seuratTime_combo_E_mat_nsprcomp <- nsprcomp(t(seuratTime_combo_E_mat),nneg=TRUE,ncomp=5)
seuratTime_combo_E_mat_mat_nsprcomp_annot <- cbind(seuratTime_combo_E_mat_nsprcomp$x,seuratTime_combo_E@meta.data)

# M 
seuratTime_combo_M_mat_nsprcomp <- nsprcomp(t(seuratTime_combo_M_mat),nneg=TRUE,ncomp=5)
seuratTime_combo_M_mat_mat_nsprcomp_annot <- cbind(seuratTime_combo_M_mat_nsprcomp$x,seuratTime_combo_M@meta.data)

# Cluster 1 #
seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1 <- seuratTime_combo_E_mat_mat_nsprcomp_annot[row.names(seuratTime_combo_E_mat_mat_nsprcomp_annot) %in% row.names(seurat_cluster_integrated_1),]
dim(seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1)
seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster1 <- seuratTime_combo_M_mat_mat_nsprcomp_annot[row.names(seuratTime_combo_M_mat_mat_nsprcomp_annot) %in% row.names(seurat_cluster_integrated_1),]
dim(seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster1)

timecourse_Cluster1_lm_fit <- lm(as.numeric(seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1$Time) ~ seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1$PC1 + seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster1$PC1)
summary(timecourse_Cluster1_lm_fit)

# Drop Later time points #
seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1_Early <- seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1[seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1$Time < 4,]
seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster1_Early <- seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster1[seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster1$Time < 4,]

timecourse_Cluster1Early_lm_fit <- lm(as.numeric(seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1_Early$Time) ~ seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1_Early$PC1 + seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster1_Early$PC1)
summary(timecourse_Cluster1Early_lm_fit)

plot_data <- as.data.frame(cbind(seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1$PC1,seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster1$PC1,seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster1$Time))
plot_data <- plot_data[plot_data$V3 < 4,] # Filter times with few samples
colnames(plot_data) <- c("Egenes","Mgenes","key")
plot_data$key_label <- factor(plot_data$key)

manual_colors <- hue_pal()(4)
base_plot_centers <- gg_means_plot(plot_data,4,-7.5,0,-5,7.5)
base_plot_centers

# Cluster 6 #
seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6 <- seuratTime_combo_E_mat_mat_nsprcomp_annot[row.names(seuratTime_combo_E_mat_mat_nsprcomp_annot) %in% row.names(seurat_cluster_integrated_6),]
dim(seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6)
seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster6 <- seuratTime_combo_M_mat_mat_nsprcomp_annot[row.names(seuratTime_combo_M_mat_mat_nsprcomp_annot) %in% row.names(seurat_cluster_integrated_6),]
dim(seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster6)

timecourse_Cluster6_lm_fit <- lm(as.numeric(seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6$Time) ~ seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6$PC1 + seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster6$PC1)
summary(timecourse_Cluster6_lm_fit)

# Drop Later time points #
seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6_NoZero <- seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6[seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6$Time > 0,]
seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster6_NoZero  <- seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster6[seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster6$Time > 0,]

timecourse_Cluster6NoZero_lm_fit <- lm(as.numeric(seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6_NoZero$Time) ~ seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6_NoZero$PC1 + seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster6_NoZero$PC1)
summary(timecourse_Cluster6NoZero_lm_fit)

plot_data <- as.data.frame(cbind(seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6$PC1,seuratTime_combo_M_mat_mat_nsprcomp_annot_Cluster6$PC1,seuratTime_combo_E_mat_mat_nsprcomp_annot_Cluster6$Time))
plot_data <- plot_data[plot_data$V3 > 0,] # Filter times with few samples
colnames(plot_data) <- c("Egenes","Mgenes","key")
plot_data$key_label <- factor(plot_data$key)

manual_colors <- hue_pal()(5)
base_plot_centers <- gg_means_plot(plot_data,5,-5,7.5,-5,5)
base_plot_centers


time_6 <- row.names(Time_integration.combined@meta.data[Time_integration.combined@meta.data$seurat_clusters==6,])
for (n in 0:12){
  seurat_cluster_integrated <- integration.combined.All@meta.data[integration.combined.All@meta.data$seurat_clusters==n,]
  overlap <- intersect(time_6,row.names(seurat_cluster_integrated))
  print(length(overlap)/length(time_6))
}
